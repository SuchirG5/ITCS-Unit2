...
public class HailHydra extends JPanel 
{
	...
	private Graphics g;
	private Timer timer;
	private ArrayList<JumpingBall> jb;
    private Ball ball; 
	...
	public HailHydra() 
    {
        ...
        jb = new ArrayList<JumpingBall>();
        jb.add(new JumpingBall((int)(Math.random()*WIDTH), (int)(Math.random()*HEIGHT), 40, new Color((int)(Math.random() * 255), 
	...
	private class TimerListener implements ActionListener 
    {
		...
		public void actionPerformed(ActionEvent e) 
        {
            ...
            {
                hits+=jb.get(i).getHits();
                if(jb.get(i).intersectWith(ball))
                {
                    jb.get(i).setX(5000);
                    jb.add(new JumpingBall((int)(Math.random()*WIDTH), (int)(Math.random()*HEIGHT), (int)(jb.get(i).getDiameter()/2), jb.get(i).getColor()));
                    jb.add(new JumpingBall((int)(Math.random()*WIDTH), (int)(Math.random()*HEIGHT), (int)(jb.get(i).getDiameter()/2), jb.get(i).getColor()));
                    

                }
                if(jb.get(i).getDiameter()<10)
                {
                    jb.get(i).setX(5000);
                }
                if(jb.get(i).getX() <=5000)
                {
                    jb.get(i).draw(g);
                }
            }
